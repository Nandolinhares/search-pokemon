...
const Favorite: React.FC = () => {
  ...
  const handleRemove = (pokemonSelected: any): void => {
    setState({
      ...state,
      favoriteList: state.favoriteList.filter((pokemon: any) => pokemonSelected !== pokemon)
    })
  }
    ...
    <Paper elevation={4} className={Styles.paper}>
      ...
      {state.favoriteList.length === 0 ? (
        ...
        <section className={Styles.section}>
          {state.favoriteList.map((pokemon: any, index: number) => (
            <Paper key={Math.random() * 1000} elevation={3} className={Styles.paperSecondary}>
              ...
              <Button variant="contained" color="secondary" className={Styles.button} onClick={() => handleRemove(pokemon)}>Remover</Button>
            </Paper>
          ))}
        </section>
      )}
